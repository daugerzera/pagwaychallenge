...
describe('BATERIA DE TESTES PARA O SERVIÇO DE CRIAÇÃO DE RECEBÍVEIS', () => {
  ...
  describe('persiste os valores corretos', () => {
    test('transactionId deve ser o mesmo do argumento', async () => {
      const cashout = mkCashout({
        persistCashout: (transactionId, statusRecebivel, dataPagamentoRecebivel, valorLiquitoRecebivel) => {
    ...
    test('valorLiquitoRecebivel deve ser 5% menor do argumento', async () => {
      const cashout = mkCashout({
        persistCashout: (transactionId, statusRecebivel, dataPagamentoRecebivel, valorLiquitoRecebivel) => {
          const percent = (dadosRecebivelOK.valorTransacao - valorLiquitoRecebivel) / dadosRecebivelOK.valorTransacao
